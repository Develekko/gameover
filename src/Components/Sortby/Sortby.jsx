...
export default function Sortby() {
  let {path} = useParams();
    ...
    const [sortgames, setSortedGames] = useState([]);
    ...
    return <>
        <div className="container text-center">
            <div className="row">
                {sortgames.length > 0 ? sortgames.slice(0, count).map((game, index) =>
            ...
            {count > sortgames.length ? null : <button onClick={moreGames} className='btn btn-outline-secondary'>More Games</button>}
        </div>
    </>
}