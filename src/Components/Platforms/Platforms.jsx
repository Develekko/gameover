...
export default function Platforms() {
  ...
  const [platform, setPlatorms] = useState([]);
  ...
  return <>
    <div className="container text-center">
      <div className="row">
        {platform.length > 0 ? platform.slice(0, count).map((game, index) =>
      ...
      {count > platform.length ? null : <button onClick={moreGames} className='btn btn-outline-secondary'>More Games</button>}
    </div>
  </>
}